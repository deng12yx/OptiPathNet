...
class DelayDetector(app_manager.RyuApp):
    ...
    def get_link_delay(self):
        ...
        for src_sport_dst in self.src_dstDelay.keys():
            src, sport, dst = tuple(map(eval, src_sport_dst.split("-")))
            s_d_delay = self.src_dstDelay[src_sport_dst] - (
                    self.echoDelay[src] + self.echoDelay[dst]) / 2;
    ...
    def show_delay(self):
        ...
        for key, val in self.echoDelay.items():
            print("s%d----%.12f" % (key, val))
        print("-----------------------show LLDP delay-----------------------")
        for key, val in self.src_dstDelay.items():
            print("%s----%.12f" % (key, val))
