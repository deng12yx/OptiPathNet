...
from test_demo import operator
...
class GlobalParameter:
    ...
    def __init__(self, m, n=100, eva=10000, decs=None, operator=None, pro=None, run=None):
        self.m = m
        self.n = n
        self.pro = pro  # Initialize the class of problem
...
class DelayGANProblem():
    def __init__(self, d, m, delayPredict, delayIfAdd, delayNewArray, delayPreArray, shortest_path, leaf, another_node,
                 arraylength):
        self.d = d  # 决策变量的维度
        self.m = m  # 目标数量
        self.lower = np.array([0] * d)  # 决策变量的下界
        self.upper = np.array([1] * d)  # 决策变量的上界
    ...
    def fit(self, mode=None, n=None, in_value=None):
        if mode == 'init':
            pop_dec = np.random.uniform(self.lower, self.upper, (n, self.d))
            pop_obj = self.objective(pop_dec)
            # print(f"[pop_dec, pop_obj] is {[pop_dec, pop_obj]}")
    ...
    def objective(self, dec):
        ...
        g2 = [item - 3 for item in f2]
        out = {"F": np.vstack((f1, f2, f3)).T, "G": np.vstack((g1, g2)).T}
        return out

    def IGD(self, pop_obj):
        ref_points = self.generate_reference_points()
        # 检查 pop_obj 和 ref_points 的形状
        # print(f"pop_obj is {pop_obj}")
        assert pop_obj.shape[1] == ref_points.shape[1], "目标函数输出的维度与参考点的维度不匹配"

        # 计算每个解与所有参考点之间的距离
        dist = np.min(np.linalg.norm(pop_obj[:, None, :] - ref_points[None, :, :], axis=2), axis=0)
        igd = np.mean(dist)
        return igd

    def generate_reference_points(self):
        ref_points = np.mgrid[0:1:100j, 0:1:100j, 0:1:100j].reshape(3, -1).T
        return ref_points
...
def process_task(i, delayNewArray, delayPreArray, shortest_path, leaf, another_node):
    ...
    nx.write_graphml(all_G[0], "img/gan_pre.graphml")
    nx.write_graphml(all_G[1], "img/gan_lat.graphml")
    max_weight_edge = []
    for G in all_G:
...
class NewdelayCompute:
    def __init__(self, weight_info, delay_info):
        ...
        self.delayPreArray = read_list_from_txt("./1/Claranet/delayArray_Claranet_10.0.txt")
        self.delayNewArray = read_list_from_txt("./1/Claranet/newDelayArray_Claranet_10.0.txt")
        self.arrayLen = [140]
        self.allF = []
        self.allTime = []
        self.shortest_path = {}
    ...
    def newDelayCompute(self):
        ...
        d = sum(value for value in delayIfAdd.values())
        delay_gan_pro = DelayGANProblem(d, 3, delayPredict, delayIfAdd, self.delayNewArray, self.delayPreArray,
                                        self.shortest_path, self.leaf, self.another_node, 40)
        delay_gan_pro.lower = np.array([0] * delay_gan_pro.d)
        delay_gan_pro.upper = np.array([maxDelay] * delay_gan_pro.d)
        for eva in [50]:
            for n in [20]:
                gp = GlobalParameter(
                    m=3,
                ...
                start = time.time()
                try:
                    population, score = gmoea.run()
                except Exception as e:
                    print(f"Exception occurred for n = {n}: {e}")
                    continue  # 继续下一个循环
                end = time.time()
                # 输出结果
                # print("Final Population:", population)
                # print(f"n is {n}, population is {population[1]}, time is {end - start}")
                # with(open("result_middle.json", "a")) as f:
                #     json.dump({"n": n, "eva": eva, "score": list(population[1][0]), "time": end - start}, f)
                #     f.write("\n")
        ...
        #     if i < len(self.delayPreArray):
        #         delay_dict = self.delayPreArray[i]
        #         # print(f"res.X[0] is {res.X[0]}")
        #         update_values = [delay_dict[key] + population[0][0][count] for count, key in enumerate(update_keys)]
        #         no_update_values = [delay_dict[key] for key in no_update_keys]
        #         updated_values = update_values + no_update_values
        #         self.delayNewArray[i] = {key: value for key, value in zip(update_keys + no_update_keys, updated_values)}
        #
...
if __name__ == '__main__':
    weight_info = read_list_from_txt("./1/Claranet/weight_info_Claranet_1.0.txt")
    delay_info = read_list_from_txt("./1/Claranet/delayinfo_Claranet_1.0.txt")
