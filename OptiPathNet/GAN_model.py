...
class Generator(nn.Module):
    ...
    def __init__(self, d, n_noise):  # 1-d vector
        ...
        self.linear3_2 = nn.Linear(d, 1, bias=True)  # 输出层形状调整为 (d, 1)
        self.bn3 = nn.BatchNorm1d(d)
        self.bn3_2 = nn.BatchNorm1d(1)
        self.d = d

    # forward method
    def forward(self, noise):
        if noise.size(0) == 1:
            noise = noise.repeat(self.d, 1)  # 如果批次大小为1，则重复两次
            x = torch.tanh(self.bn1(self.linear1(noise)))
            x = torch.tanh(self.bn2(self.linear2(x)))
            x = torch.sigmoid(self.bn3_2(self.linear3_2(x)))
        else:
            x = torch.tanh(self.bn1(self.linear1(noise)))
            x = torch.tanh(self.bn2(self.linear2(x)))
...
class Discriminator(nn.Module):
    ...
    def __init__(self, d):
        ...
        self.linear1_2 = nn.Linear(1, d, bias=True)
